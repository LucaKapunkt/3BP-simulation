...
const CameraControls: React.FC = () => {
  ...
  // Funktion für den Fokus-Wechsel basierend auf der aktuellen Kameraposition
  const handleFokusChange = () => {
    setCamFokus(prev => {
      if (camPos === 'FPV' || camPos === '3PV') {
        // Für FPV und 3PV nur zwischen Ohne und Bewegungsrichtung wechseln
        return prev === 'Ohne' ? 'Bewegungsrichtung' : 'Ohne';
      } else {
        // Für Frei-Modus: Bewegungsrichtung überspringen
        switch(prev) {
  ...
  // Effekt für automatische Fokus-Änderungen bei Positionswechsel
  useEffect(() => {
    if (camPos === 'Frei') {
      setCamFokus('Ohne');
